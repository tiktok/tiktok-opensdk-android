...
import com.tiktok.sdk.demo.auth.userinfo.UserInfoQuery
...
class MainViewModel(
            ...
            if (it.isEditable) {
                ...
                sendViewEffect(
                    ViewEffect.ShowGeneralAlert(
                        R.string.invalid_scope_auth,
                        R.string.invalid_scope_auth_desc
                    )
                )
                scopeStates[scopeType] = it.copy(isOn = !isOn)
            }
    ...
    fun getUserBasicInfo(authCode: String, grantedPermissions: String, codeVerifier: String) {
        ...
        UserInfoQuery.getAccessToken(authCode, BuildConfig.REDIRECT_URL, codeVerifier) { response, errorMsg ->
            errorMsg?.let {
                sendViewEffect(
                    ViewEffect.ShowAlertWithResponseError(
                        R.string.access_token_error,
                        errorMsg
                    )
                )
            ...
            response?.let { accessTokenInfo ->
                UserInfoQuery.getUserInfo("${accessTokenInfo.tokenType} ${accessTokenInfo.accessToken}") { userInfo, errorMessage ->
                    ...
                    userInfo?.let {
                        sendViewEffect(
                            ViewEffect.GettingUserInfoSuccess(
                                grantedPermissions,
