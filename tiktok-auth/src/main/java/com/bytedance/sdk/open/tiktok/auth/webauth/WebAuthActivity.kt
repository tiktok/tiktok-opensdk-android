...
internal class WebAuthActivity : Activity() {
    ...
    override fun onBackPressed() {
        if (mContentWebView.canGoBack()) {
            ...
            redirectToClientApp(BaseError.ERROR_CANCEL, errorMsg = USER_CANCEL_AUTH)
        }
    }
    ...
    private fun sendInnerResponse(webAuthRequest: WebAuthRequest, resp: Auth.Response): Boolean {
    ...
    private fun initView() {
        ...
        mCancelBtn.setOnClickListener { onCancel(BaseError.ERROR_CANCEL) }
    ...
    private fun parseErrorAndRedirectToClient(uri: Uri) {
        ...
        val errorCode = try {
            errorCodeStr?.toInt() ?: BaseError.ERROR_UNKNOWN
        } catch (e: Exception) {
            BaseError.ERROR_UNKNOWN
        }
    ...
    private fun handleRedirect(url: String): Boolean {
        ...
        val code = uri.getQueryParameter(Keys.WebAuth.REDIRECT_QUERY_CODE) ?: ""
        val state = uri.getQueryParameter(Keys.WebAuth.REDIRECT_QUERY_STATE)
        val grantedPermissions = uri.getQueryParameter(Keys.WebAuth.REDIRECT_QUERY_SCOPE) ?: ""
        if (TextUtils.isEmpty(code)) {
            parseErrorAndRedirectToClient(uri)
            return false
        }
        redirectToClientApp(BaseError.OK, code, state, grantedPermissions)
        return true
    }
    ...
    companion object {
        ...
        private const val REDIRECT_URL = "https://${WEB_AUTH_HOST}$WEB_AUTH_REDIRECT_URL_PATH"
    }
}
