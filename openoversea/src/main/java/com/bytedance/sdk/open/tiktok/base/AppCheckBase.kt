...
internal abstract class AppCheckBase(open val context: Context) : IAppCheck {
    ...
    override val isShareSupported: Boolean
        get() = isAppSupportAPI(sharePackageName, BuildConfig.TIKTOK_SHARE_ACTIVITY, Keys.API.MIN_SDK_NEW_VERSION_API)

    override val isAppInstalled: Boolean
        get() = isAppSupportAPI(appPackageName, BuildConfig.TIKTOK_AUTH_ACTIVITY, Keys.API.AUTH_REQUIRE_API)

    override val isShareFileProviderSupported: Boolean
        get() = isAppSupportAPI(sharePackageName, BuildConfig.TIKTOK_SHARE_ACTIVITY, Keys.API.SHARE_SUPPORT_FILEPROVIDER)
    ...
    private fun platformSdkVersion(): Int {
        return try {
            val componentName = ComponentName(appPackageName, componentClassName(appPackageName, BuildConfig.TIKTOK_AUTH_ACTIVITY))
            context.packageManager.getActivityInfo(componentName, PackageManager.GET_META_DATA).metaData?.getInt(Keys.SDK_VERSION_KEY, Keys.SDK_VERSION_ERROR) ?: Keys.SDK_VERSION_ERROR
        } catch (e: PackageManager.NameNotFoundException) {
            Keys.SDK_VERSION_ERROR
        }
    }

    override val sharePackageName: String = BuildConfig.TIKTOK_COMPONENT_PATH
}
