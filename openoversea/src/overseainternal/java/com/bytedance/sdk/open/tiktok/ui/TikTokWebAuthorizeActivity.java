...
public class TikTokWebAuthorizeActivity extends BaseWebAuthorizeActivity {
    ...
    @Override
    protected void sendInnerResponse(Auth.Request req, Base.Response resp) {
        if (resp != null && mContentWebView != null) {
            if (resp.getExtras() == null) {
                resp.setExtras(new Bundle());
            }
            resp.getExtras().putString(WAP_AUTHORIZE_URL, mContentWebView.getUrl());
        }
